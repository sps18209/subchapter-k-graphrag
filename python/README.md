...
- `subk_see.py` / `subk_disguised.py` / `subk_doctrine.py` / `subk_intake.py` / `subk_analyze.py` — fact-intensive **doctrine analyzer**. Wired so far: **substantial economic effect** (IRC 704(b)) and **disguised sale** (IRC 707(a)(2)(B); Reg. 1.707-3 to -5, incl. the 2-year presumption and the 10-factor F&C test). `subk_analyze.py --capabilities` states exactly what can/can't be ingested; the tool reports which factors of the picked doctrine it can and cannot reach from the facts given. Each doctrine module exports a uniform shape (factor tree + fact-frame + scope signals + provision patterns) and is added by registering in `subk_doctrine.DOCTRINES`. Pick with `--doctrine see|disguised_sale` or let autodetect choose by issue text / form field names. `subk_intake` is the read-only ingestion engine
