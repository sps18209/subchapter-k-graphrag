...
- `redact.py` — deterministic, **code-based name redaction** before any send: the interview captures the REAL name (John Doe), derives a code (JoDo) kept in a LOCAL map, and the tool find-replaces every occurrence (incl. in documents) with the code; a narrow local `scan_candidates` heuristic (honorifics / `X v. Y` captions / signature blocks / name-pairs, legal terms filtered) surfaces UN-declared names at ingest for you to label before sending
