...
- `mask.py` — local, reversible **identifier masking** (defense-in-depth): client identifiers in the FACT items (entity names, EIN/SSN, amounts, email/phone) are tokenized before anything reaches the model and restored locally for display; the map never leaves the machine, public LAW text is never masked
