...
- `subk_see.py` / `subk_disguised.py` / `subk_antiabuse.py` / `subk_doctrine.py` / `subk_intake.py` / `subk_analyze.py` — fact-intensive **doctrine analyzer**. Wired so far: **substantial economic effect** (IRC 704(b)), **disguised sale** (IRC 707(a)(2)(B); Reg. 1.707-3 to -5 incl. the 2-year presumption and the 10-factor F&C test), and the **partnership anti-abuse rule** (Treas. Reg. 1.701-2 — 3 intent-of-Subchapter-K predicates + the principal-purpose test + 7 F&C factors + the aggregate-vs-entity remedy). `subk_analyze.py --capabilities` states exactly what can/can't be ingested; the tool reports which factors of the picked doctrine it can and cannot reach from the facts given. Each doctrine module exports a uniform shape (factor tree + fact-frame + scope signals + provision patterns + doctrine-aware framing + interview script) and is added by registering in `subk_doctrine.DOCTRINES` — no spine edits required. Pick with `--doctrine see|disguised_sale|anti_abuse` or let autodetect choose by issue text / form field names. `subk_intake` is the read-only ingestion engine
...
- `redact.py` — deterministic **name→ROLE redaction** before any send: the interview captures each party's REAL name (kept in a LOCAL map) and maps it to its FUNCTIONAL ROLE — `John Doe` → `Contributing partner` — then find-replaces every occurrence (incl. in documents) with the role, so the graph represents parties by *what they are*, never a name or code. A narrow local `scan_candidates` heuristic (honorifics / `X v. Y` captions / signature blocks / name-pairs, legal terms filtered) surfaces UN-declared names at ingest for you to label before sending
- `mask.py` — local, reversible **identifier masking** (backstop): EIN/SSN, amounts, email/phone in the FACT items are tokenized before anything reaches the model and restored locally for display; the map never leaves the machine, public LAW text is never masked
